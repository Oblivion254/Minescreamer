...
var timerId;
var winner;
var scaryAmbienceAudio = new Audio('audio/scary-ambience.mp3');
scaryAmbienceAudio.onerror = function() {
  console.log('Audio file failed to load. Check the file path.');
};
var ambienceAudioStarted = false;
...
boardEl.addEventListener('click', function(e) {
  ...
  if (clickedEl.classList.contains('game-cell')) {
    ...
    if (e.shiftKey && !cell.revealed && bombCount > 0) {
      ...
      hitBomb = cell.reveal();
      if (hitBomb) {
        scaryAmbienceAudio.pause();
        scaryAmbienceAudio.currentTime = 0;
        const jumpscareAudio = new Audio('audio/cringe-jumpscare.mp3');
        jumpscareAudio.play();
        var popup = document.getElementById('gameover-pop-up');
...
function ambienceAudio() {
  if (!ambienceAudioStarted) {
  scaryAmbienceAudio.play();
  scaryAmbienceAudio.loop = true;
  audioStarted = true;
}
  }
